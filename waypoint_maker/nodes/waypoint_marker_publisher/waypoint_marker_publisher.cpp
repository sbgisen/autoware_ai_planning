...
bool isStopForStopline(const autoware_msgs::Waypoint& wp)
{
  // TODO: replace with actual condition, e.g. using wp.wpstate.event_state
  // return (wp.wpstate.event_state == autoware_msgs::WaypointState::TYPE_STOPLINE);
  return false;
}

bool isStopForObstacle(const autoware_msgs::Waypoint& wp)
{
  // TODO: replace with actual condition
  return false;
}

void createLocalStopReasonMarkers(const autoware_msgs::Lane& lane_waypoint)
{
  if (lane_waypoint.waypoints.empty())
    return;
  ...
  for (const auto& wp : lane_waypoint.waypoints)
  {
    ...
    geometry_msgs::Point base = wp.pose.pose.position;
    base.z = base_z;

    if (isStopForStopline(wp))
    {
      // white bar
      ...
      m.ns = "stop_reason";
      m.id = wp.pose.pose.position.x * 1000;  // or any unique id logic
      m.type = visualization_msgs::Marker::CUBE;
      m.action = visualization_msgs::Marker::ADD;
      m.frame_locked = true;

      m.pose = wp.pose.pose;
      m.pose.position.z = base.z + 0.4;
      m.scale.x = 0.5;
      m.scale.y = 0.05;
      m.scale.z = 0.3;
      ...
      g_local_waypoints_marker_array.markers.push_back(m);
    }
    else if (isStopForObstacle(wp))
    {
      // red block
      visualization_msgs::Marker m;
      m.header.frame_id = "map";
      m.header.stamp = ros::Time::now();
      m.ns = "stop_reason";
      m.id = wp.pose.pose.position.y * 1000;
      m.type = visualization_msgs::Marker::CUBE;
      m.action = visualization_msgs::Marker::ADD;
      m.frame_locked = true;

      m.pose = wp.pose.pose;
      m.pose.position.z = base.z + 0.6;
      m.scale.x = 0.4;
      m.scale.y = 0.4;
      m.scale.z = 0.4;
