...
AstarSearch::AstarSearch()
{
  ...
  private_nh_.param<double>("circulation_angle_limit", circulation_angle_limit_, 1.75 * M_PI);
...
bool AstarSearch::search()
{
  ...
  while (!openlist_.empty())
  {
    ...
    if (isGoal(current_an->x, current_an->y, current_an->theta))
    {
      ...
      ROS_DEBUG("move_distance = %lf, move_angle = %lf, theta = %lf", current_an->move_distance, current_an->move_angle,
                current_an->theta);
      setPath(top_sn);
      return true;
    }
    ...
    for (const auto& state : state_update_table_[top_sn.index_theta])
    {
      ...
      if ((enable_path_angle_limit_ && move_angle > path_angle_limit_) ||
          ...
          (prohibit_circulation_ && fabs(next_theta) > circulation_angle_limit_))
      {
        continue;
      }
